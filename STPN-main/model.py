...
class STPN(nn.Module):
    ...
    def __init__(self, h_layers, in_channels, hidden_channels, out_channels, emb_size, dropout, wemb_size=4, time_d=4, heads=4, support_len=3, order=2, num_weather=8, use_se=True, use_cov=True):
        ...
        if self.use_cov:
            self.convs.append(STMH_GCNN_layer(
                in_channels + wemb_size, hidden_channels[0], emb_size, dropout, time_d, heads, support_len, order, False))
            self.w_embedding = nn.Embedding(num_weather, wemb_size)
        else:
            self.convs.append(STMH_GCNN_layer(
                in_channels, hidden_channels[0], emb_size, dropout, time_d, heads, support_len, order, False))
        for i in range(h_layers):
            if self.use_se:
                self.se.append(SELayer(hidden_channels[i]))
            self.convs.append(STMH_GCNN_layer(
                hidden_channels[i], hidden_channels[i+1], emb_size, dropout, time_d, heads, support_len, order, False))
        self.final_conv = STMH_GCNN_layer(
            hidden_channels[h_layers], out_channels, emb_size, dropout, time_d, heads, support_len, order, True)
