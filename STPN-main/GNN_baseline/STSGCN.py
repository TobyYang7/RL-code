...
class ST_GCNN_layer(nn.Module):
    def __init__(self,
        ...
        self.gcn = ConvTemporalGraphical(
            time_dim, joints_dim, in_channels)  # the convolution layer
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.txcnns.append(
            CNN_layer(input_time_frame, output_time_frame, txc_kernel_size, txc_dropout))
        for i in range(1, n_txcnn_layers):
            self.txcnns.append(
                CNN_layer(output_time_frame, output_time_frame, txc_kernel_size, txc_dropout))
