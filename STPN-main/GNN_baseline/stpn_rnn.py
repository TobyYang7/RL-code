...
class STPN(nn.Module):
    ...
    def __init__(self, out_len, h_layers, in_channels, hidden_channels, out_channels, dropout, wemb_size=4, support_len=3, order=2, num_weather=8):
        ...
        self.convs.append(STGLSTM_layer(in_channels + wemb_size,
                          hidden_channels[0], dropout, support_len, order))
        self.w_embedding = nn.Embedding(num_weather, wemb_size)
        for i in range(h_layers):
            self.se.append(SELayer(hidden_channels[i]))
            self.convs.append(STGLSTM_layer(
                hidden_channels[i], hidden_channels[i+1], dropout, support_len, order))
        ...
        self.out_conv = STGLSTM_layer(
            hidden_channels[h_layers], out_channels, dropout, support_len, order)
    ...
    def forward(self, x, supports, w_type):
        ...
        for i in range(self.h_layers + 1):
            ...
            if i < self.h_layers:
                x = self.se[i](x)
        out = self.out_conv(
            x, supports, out_len=self.out_len, final_layer=True)
        return out
