...
class STPN(nn.Module):
    ...
    def __init__(self, in_len, out_len, h_layers, in_channels, hidden_channels, out_channels, dropout, wemb_size=4, support_len=3, order=2, num_weather=8, kernel=3):
        ...
        self.convs.append(STGTCN_layer(in_channels + wemb_size,
                          hidden_channels[0], dropout, support_len, order, kernel))
        ...
        for i in range(h_layers):
            ...
            self.convs.append(STGTCN_layer(
                hidden_channels[i], hidden_channels[i+1], dropout, support_len, order, kernel))
