...
def var_predict_svr(np_, out_len=12, in_len=36, test_ratio=0.2, kernel='linear', C=1.0, epsilon=0.1):
    """
    ...
    for route in trange(n_route):
        ...
        for i in range(in_len, n_sample - n_train - out_len):
            ...
            if len(X_test[i-in_len]) != in_len:
                print(f"Anomaly at index {i}. Expected length {in_len}, but got {len(X_test[i-in_len])}.")
